...
var class_decorator_downlevel_transformer_1 = require("./class_decorator_downlevel_transformer");
var decorator = require("./decorator-annotator");
var decorators_1 = require("./decorators");
var es5processor = require("./es5processor");
var fileoverview_comment_transformer_1 = require("./fileoverview_comment_transformer");
...
var Annotator = (function (_super) {
    ...
    function Annotator(typeChecker, file, host, tsHost, tsOpts, sourceMapper) {
        ...
        _this.namespaceList = [];
        _this.nowClass = undefined;
        _this.nowClassHeritageClause = [];
    ...
    Annotator.prototype.maybeProcess = function (node) {
        ...
        switch (node.kind) {
            ...
            case ts.SyntaxKind.MethodDeclaration:
                fnDecl = node;
                tags = decorators_1.hasExportingDecorator(node, this.typeChecker) ? [{ tagName: 'export' }] : [];
                this.emitFunctionType([fnDecl], tags);
                var functionName = '';
                if (fnDecl.name) {
                    functionName = fnDecl.name.getText().trim();
                }
                var isStatic = hasModifierFlag(fnDecl, ts.ModifierFlags.Static);
            ...
            case ts.SyntaxKind.VariableStatement:
                ...
                if (docTags.length > 0 && node.getFirstToken()) {
                    ...
                    var isEnum = docTags.some(function (t) { return t.tagName === 'enum'; });
                    var statement_1 = node;
                    if (isEnum && statement_1.declarationList.declarations) {
                        var text = node.getFullText();
                        var comments = ts.getLeadingCommentRanges(text, 0);
                        // this.writeRange(node, node.getStart(), statement.declarationList.declarations[0].getStart());
                        this.emit(this.namespaceList.join('.') + '.');
                        this.writeNodeFrom(node, statement_1.declarationList.declarations[0].getStart());
                    }
                    else {
            ...
            case ts.SyntaxKind.ModuleDeclaration:
                var myModule = node;
                var namespaceName = myModule.name.getFullText().trim();
                this.namespaceList.push(namespaceName);
                this.visit(myModule.body);
    ...
    Annotator.prototype.visitClassDeclaration = function (classDecl) {
        ...
        var classHeritage = classDecl.heritageClauses;
        if (classHeritage && classHeritage.length > 0) {
            for (var index = 0; index < classHeritage.length; index++) {
                var heritage = classHeritage[index];
                if (heritage.types && heritage.types.length > 0) {
                    for (var i = 0; i < heritage.types.length; i++) {
                        var type = heritage.types[i];
        ...
        ts.forEachChild(classDecl, function (child) {
